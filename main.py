...
async def init_db():
    ...
    # check columns in users and add if missing (future-proof)
    async with db.execute("PRAGMA table_info(users);") as cur:
        rows = await cur.fetchall()
    existing_cols = [r[1] for r in rows]  # second field is column name

    if "free_searches" not in existing_cols:
        logger.info("Migration: adding 'free_searches' column to users")
        await db.execute(f"ALTER TABLE users ADD COLUMN free_searches INTEGER DEFAULT {FREE_DM_SEARCHES};")
        await db.commit()

    # Ensure owner and sudo records exist
    async with db.execute("SELECT 1 FROM users WHERE user_id = ?", (OWNER_ID,)) as cur:
        if not await cur.fetchone():
            await db.execute(
                "INSERT OR IGNORE INTO users (user_id, credits, free_searches, joined_at, is_admin, is_sudo) VALUES (?, ?, ?, ?, ?, ?)",
...
class DataTraceBot:
    ...
    async def process_lookup_request(self, user_id: int, query: str, lookup_type: str, update_obj: Update):
        ...
        # credits handling
        cost_to_deduct = 0
        if userrec.get("credits", 0) >= cost:
            cost_to_deduct = cost
        else:
            # allow free searches in private until exhausted
            if update_obj.effective_chat.type == "private" and userrec.get("free_searches", 0) > 0:
                new_free = userrec.get("free_searches", 0) - 1
                await set_user_field(user_id, "free_searches", new_free)
                cost_to_deduct = 0
            else:
                # prompt user to buy or refer
    ...
    async def cb_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "verify_join":
            ...
            if pack in mapping:
                credits, price = mapping[pack]
                await self.app.bot.send_message(LOG_SEARCH_CHANNEL,
                                                f"Purchase request by {user.id} @{user.username or ''}: {credits} credits for ₹{price}. Please confirm.")
                await query.message.reply_text(f"Payment instructions: Send ₹{price} to UPI: example@upi. After payment, notify admin.")
        elif data == "buy_credits_cb":
            ...
            if user.id not in SUDO_IDS:
                ...
                return
            # guide admins how to proceed (they should reply with the proper command)
            await query.edit_message_text("Reply to me with the action (e.g., 'add <user_id> <amount>', 'ban <user_id>').")
        else:
            await query.edit_message_text("Unknown action.")
